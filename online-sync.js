...
const LIVE_STREAM_INTERVAL_MS = 28;
...
async function publishLocalChanges() {
  ...
  try {
    ...
    console.error('ONLINE-P1.2 publish klaida', error);
...
async function initializeWorkspace() {
  try {
    ...
    console.error('ONLINE-P1.2 workspace inicijavimo klaida', error);
...
function normalizeLiveStroke(item) {
  ...
  // ONLINE-P1.2: siunčiame tik naujus taškų gabalus. Taip nereikia per kiekvieną
...
onValue(connectedRef, snapshot => {
  ...
  console.error('ONLINE-P1.2 connection klaida', error);
...
function flushLiveDelta(strokeId, force = false) {
  ...
  update(myLiveStrokeRef(stroke.id), updates).catch(error => {
    console.warn('ONLINE-P1.2 live delta klaida', error);
  });
}
...
if (location.protocol === 'file:') {
  console.info('ONLINE-P1.2 veikia su Firebase ir iš lokalaus failo, tačiau bendrinama file:// nuoroda kitame kompiuteryje neveiks. Patalpinkite aplanką statiniame hostinge.');
}
