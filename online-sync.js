...
// P1.7.9.14 vienkartinė saugi korekcija po P1.7.5.6 migracijos.
...
async function applyP17913LegacyScheduleRoomRepairOnce() {
  ...
  for (const [dateKeyRaw, run] of Object.entries(runs)) {
    ...
    try {
      ...
      console.error('P1.7.9.14 istorinio Room susiejimo klaida', error);
    } finally {
...
if (teacherProfileRef) {
  onValue(teacherProfileRef, snapshot => {
    ...
    applyP17913LegacyScheduleRoomRepairOnce().catch(error => console.warn('P1.7.9.14 istorinio Room pataisa neįvykdyta', error));
  }, error => {
...
window.addEventListener('p2:schedule-request', async event => {
  ...
  try {
    ...
    console.error('P2-SPLIT-P2.5-P4-P1.7.9.14 tvarkaraščio įrašymo klaida', error);
