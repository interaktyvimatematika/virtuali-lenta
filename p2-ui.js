(() => {
  ...
  // M1.4: Students modal/card markup lives in p2-students-ui.js.
  // Student actions, Room navigation, schedule and Firebase communication stay here.
  const studentsUi = window.P772StudentsUI;
  if (!studentsUi || typeof studentsUi.createStudentsModal !== 'function' || typeof studentsUi.renderStudentsBodyMarkup !== 'function') {
    console.error('P2 mokinių UI modulis nerastas');
    return;
  }

  // M1.5: Schedule modal/week/editor markup lives in p2-schedule-ui.js.
  // Schedule actions, Room creation and Firebase communication remain here.
  const scheduleUi = window.P772ScheduleUI;
  if (!scheduleUi || typeof scheduleUi.createScheduleModal !== 'function' || typeof scheduleUi.renderScheduleWeekMarkup !== 'function' || typeof scheduleUi.renderScheduleCreateEditorMarkup !== 'function' || typeof scheduleUi.renderScheduleEditEditorMarkup !== 'function') {
  ...
  function taskDiagramMarkup(task) {
    ...
    if (type === 'centroid') return wrap('Trikampio pusiaukraštinės ir jų susikirtimo taškas', `
    ...
    if (type === 'cyclic-quadrilateral') return wrap('Į apskritimą įbrėžtas keturkampis', `
      ...
      <path d="M62.22 56.56 A20 20 0 0 0 86.08 33.53" class="guide"/>
      <path d="M163.43 117.69 A18 18 0 0 0 147.79 130.36" class="guide"/>
      ...
      <text x="82" y="64">112°</text><text x="142" y="114">?</text>`);
    if (type === 'included-angle') return wrap('Trikampis su dviem kraštinėmis ir kampu tarp jų', `
      ...
      <text x="82" y="94">8</text><text x="116" y="151">10</text><text x="64" y="128">30°</text>`);
    ...
    if (type === 'sine-law') return wrap('Trikampis sinusų teoremai', `
      ...
      <text x="57" y="102">30°</text><text x="84" y="17">10</text><text x="91" y="89">?</text>`);
    return '';
  }
  ...
  function ensureStudentsModal() {
    ...
    studentsModal = studentsUi.createStudentsModal(document);
  ...
  function renderStudentsModal() {
    ...
    const renderedStudents = studentsUi.renderStudentsBodyMarkup({
      students,
      visibleStudents,
      roomId,
      linkedStudentId,
      ...
      history,
      currentLesson,
      selectedOwnsActiveRoom,
      defaultLessonId,
      selectedClassParticipant,
      studentGradeFilter,
      studentSearchQuery,
      studentCreateOpen,
      studentEditOpen,
      ...
      studentGuardianLabel,
      studentGuardianRelation,
      formatStudentDate,
      studentLessonOccurrenceState,
      studentUpcomingScheduleLessons,
      studentPracticeLabel,
      linkedClassSessionIdForRoom,
      scheduleMode,
      scheduleModeLabel,
      studentTeacherLabel,
      studentLessonWhenLabel,
      studentLessonResultText,
      studentScheduledLessonWhenLabel,
      studentLessonStatus,
      renderStudentRoomHistoryDetails,
      escapeHtml
    });
    studentGradeFilter = renderedStudents.studentGradeFilter;
    host.innerHTML = renderedStudents.html;
  ...
  const SCHEDULE_HISTORY_MIN_DATE = '2026-08-13';

  function scheduleHistoryStartDate(value) {
    const date = scheduleDateKeyValid(value) ? String(value) : SCHEDULE_HISTORY_MIN_DATE;
    return date < SCHEDULE_HISTORY_MIN_DATE ? SCHEDULE_HISTORY_MIN_DATE : date;
  }
  ...
  function scheduleSlotTimeVersions(entry) {
    ...
    const list = Object.entries(raw).map(([id, value]) => ({ id, ...(value && typeof value === 'object' ? value : {}) }))
      ...
      .map(item => ({ ...item, effectiveFrom: scheduleHistoryStartDate(item.effectiveFrom), durationMinutes: Math.max(15, Math.min(180, Math.round(Number(item.durationMinutes) || 40))) }));
    if (!list.length || Number(entry?.slotModelVersion || 0) < 2) {
      const legacyDate = scheduleHistoryStartDate(scheduleDateKeyValid(entry?.startDate) ? entry.startDate : entry?.date);
  ...
  function scheduleAttendanceAssignments(entry) {
    ...
    Object.entries(raw).forEach(([id, value]) => {
      ...
      const item = { id, ...value, studentId, mode: scheduleMode(value) };
      if (item.mode === 'recurring') item.startDate = scheduleHistoryStartDate(item.startDate);
      result.push(item);
    });
    ...
    for (const studentId of legacyIds) {
      ...
      const startDate = scheduleHistoryStartDate(entry?.startDate);
  ...
  function renderScheduleAttendancePanel(editorHost, selectedDate, activeAssignments, run, occurrenceState, students) {
    ...
    activeAssignments.forEach(assignment => {
      ...
      row.dataset.scheduleAttendanceRowStudent = assignment.studentId;
  ...
  function scheduleAssignmentSummary(assignment) {
    const mode = scheduleMode(assignment);
    if (mode === 'recurring') return `nuo ${scheduleHistoryStartDate(assignment.startDate)}`;
    if (mode === 'dates') {
  ...
  function ensureScheduleContextStyles() {
    ...
    style.textContent = `
      ...
      #p2ScheduleWeekPane [data-schedule-card].p2-schedule-context-card {
        cursor: pointer;
        transition: box-shadow .14s ease, border-color .14s ease;
      }
      #p2ScheduleWeekPane [data-schedule-card].p2-schedule-context-card:hover {
        box-shadow: 0 5px 16px rgba(39,55,91,.10);
      }
      #p2ScheduleWeekPane .p2-schedule-card-overview-action {
        display:flex;
        align-items:center;
      ...
      #p2ScheduleWeekPane .p2-schedule-card-overview-action:hover,
      #p2ScheduleWeekPane .p2-schedule-card-overview-action.is-context-active {
        background:rgba(74,103,214,.13);
        border-color:rgba(74,103,214,.28);
      }
      ...
      #p2ScheduleEditorPane .p2-schedule-legacy-tools {
        display: grid;
      ...
      #p2ScheduleEditorPane .p2-schedule-legacy-tools label {
        display: grid;
        gap: 4px;
        margin: 0;
        color: #687287;
      ...
      #p2ScheduleEditorPane .p2-schedule-legacy-tools input {
        min-width: 0;
        height: 32px;
      ...
      #p2ScheduleEditorPane .p2-schedule-legacy-actions {
        display: flex;
        gap: 5px;
      }
      #p2ScheduleEditorPane .p2-schedule-legacy-actions button {
        min-height: 32px;
        padding: 5px 8px;
        white-space: nowrap;
      }
  ...
  function decorateScheduleContextZones(weekHost) {
    ...
    weekHost.querySelectorAll('[data-schedule-card]').forEach(card => {
      ...
      card.classList.add('p2-schedule-context-card');
      let overviewAction = card.querySelector('[data-schedule-card-overview]');
      if (!overviewAction) {
        overviewAction = document.createElement('button');
        overviewAction.type = 'button';
        overviewAction.className = 'p2-schedule-card-overview-action';
        overviewAction.dataset.scheduleCardOverview = '1';
        overviewAction.textContent = 'Pamokos informacija';
        card.appendChild(overviewAction);
      }
      overviewAction.classList.toggle('is-context-active', Boolean(selected && scheduleContextMode === 'overview'));
      const zones = [
  ...
  function enhanceLegacyScheduleAssignmentRows(editorHost, assignments) {
    ...
    assignments.forEach((assignment, index) => {
      ...
      const tools = document.createElement('div');
      tools.className = 'p2-schedule-legacy-tools';

      const label = document.createElement('label');
      const caption = document.createElement('span');
      caption.textContent = 'Pirmas lankymas';
      const input = document.createElement('input');
      input.type = 'date';
      input.min = SCHEDULE_HISTORY_MIN_DATE;
      input.value = scheduleHistoryStartDate(assignment.startDate);
      input.dataset.scheduleLegacyStart = String(assignment.id || '');
      label.append(caption, input);

      const actions = document.createElement('div');
      actions.className = 'p2-schedule-legacy-actions';
  ...
  function applyScheduleContextEditor(editorHost) {
    ...
    const targetSections = scheduleContextMode === 'assignment'
      ? [assignmentSection, lessonSection]
      : [sectionByMode[scheduleContextMode] || lessonSection];
    sections.forEach(section => {
      if (targetSections.includes(section)) section.classList.add('p2-schedule-context-focus');
  ...
  function renderScheduleModal() {
    ...
    weekHost.innerHTML = scheduleUi.renderScheduleWeekMarkup({
      weekDates,
      entries,
      todayKey,
      selectedDateKey: scheduleSelectedDateKey,
      editingScheduleId,
      editingScheduleDateKey,
      scheduleRuns: teacherStudentDb.scheduleRuns || {},
      scheduleCreatePreset,
      students,
      hasEditing: Boolean(editing),
      scheduleSlotOccursOnDate,
      scheduleSlotTimeForDate,
      scheduleStudentNames,
      scheduleOccurrenceState,
      lessonForId,
    ...
    if (scheduleCreateMode && !editing) {
      ...
      editorHost.innerHTML = scheduleUi.renderScheduleCreateEditorMarkup({
        effectiveFrom,
        day,
      ...
      editorHost.innerHTML = scheduleUi.renderScheduleEditEditorMarkup({
        selectedDate,
        time,
        assignments,
        activeAssignments,
    ...
    weekHost.querySelectorAll('[data-schedule-card]').forEach(card => card.addEventListener('click', event => {
      ...
      if (context.mode === 'assignment' && context.label) {
        ...
        const matched = active.find(item => {
          ...
          return scheduleContextLabelsMatch(scheduleStudentTeacherLabel(student, studentList()), context.label);
        });
        scheduleContextStudentId = String(matched?.studentId || '');
      }
      editingScheduleId = scheduleId;
    ...
    weekHost.querySelectorAll('[data-schedule-card-overview]').forEach(button => button.addEventListener('click', event => {
      event.stopPropagation();
      const card = button.closest('[data-schedule-card]');
      const scheduleId = String(card?.dataset.scheduleCard || '');
      const dateKey = String(card?.dataset.scheduleDate || '');
      if (!scheduleId || !scheduleDateKeyValid(dateKey)) return;
      scheduleContextMode = 'overview';
      scheduleContextAssignmentLabel = '';
      scheduleContextStudentId = '';
      editingScheduleId = scheduleId;
      editingScheduleDateKey = dateKey;
    ...
    const normalRecurringStart = editorHost.querySelector('#p2ScheduleAssignStartDate');
    if (normalRecurringStart) {
      normalRecurringStart.min = SCHEDULE_HISTORY_MIN_DATE;
      if (normalRecurringStart.value && normalRecurringStart.value < SCHEDULE_HISTORY_MIN_DATE) {
        normalRecurringStart.value = SCHEDULE_HISTORY_MIN_DATE;
      }
    }
    ...
    editorHost.querySelector('[data-schedule-assignment-add]')?.addEventListener('click', () => {
      ...
      if (mode === 'recurring') {
        ...
        if (detail.startDate < SCHEDULE_HISTORY_MIN_DATE) { toast(`Tvarkaraščio istorija prasideda ${SCHEDULE_HISTORY_MIN_DATE}`); return; }
      } else if (mode === 'dates') {
    ...
    editorHost.querySelectorAll('[data-schedule-legacy-migrate]').forEach(button => button.addEventListener('click', () => {
      ...
      const input = Array.from(editorHost.querySelectorAll('[data-schedule-legacy-start]'))
        .find(el => String(el.dataset.scheduleLegacyStart || '') === assignmentId);
